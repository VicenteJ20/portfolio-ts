...
export const Navbar = () => {
  ...
  return (
    <div className={`fixed top-0 left-0 bg-white ${menu ? 'h-screen' : ''} sm:h-fit w-screen flex flex-row items-center ${poppins.className}`}>
      <div className={`flex flex-col sm:flex-row items-center px-4 py-4 w-full ${menu ? 'absolute left-0 top-0 h-full': 'justify-between'}`}>
        ...
        <Link href='/contact' className={`border-2 font-semibold p-1 px-2 rounded-md border-gray-900 text-lg bg-gray-900 text-white hover:bg-gray-700 ${menu ? 'absolute top-36 right-6' : 'hidden'} sm:block`}>Contacto</Link>
